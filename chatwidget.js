...
window.addEventListener('onEventReceived', obj => {
    ...
    switch(event) {
        case 'message': addMessage(data.data); animateContainers(); if(!fieldData.cacheAllMessages) clearMessageLog(); break;
        case 'delete-message': deleteMessage(data.data.msgId); break;
        case 'delete-messages': deleteMessages(data.data.userId); break;
        case 'event:test': if(data.field == 'testMessage') testMessage(); break;
        case 'follower-latest': case 'subscriber-latest': case 'cheer-latest': case 'tip-latest': case 'raid-latest': case 'host-latest': if(addAlert(data, event.split('-')[0])) { animateContainers(); if(!fieldData.cacheAllMessages) clearMessageLog(); } break;
    }
});
...
window.addEventListener('onWidgetLoad', obj => {
    ...
    let colors = '';
    shownColors = Object.keys(fieldData).filter(p => p.startsWith('color') && p.endsWith('Show') && fieldData[p]).map(p => p.slice(0, -4));
    for(let i = 0; i < shownColors.length; i++) colors += `.container:nth-last-of-type(${shownColors.length}n+${i + 1}) svg { fill: ${fieldData[shownColors[i]]} }.container:nth-last-of-type(${shownColors.length}n+${i + 1}) .message-name, .container:nth-last-of-type(${shownColors.length}n+${i + 1}) .alert-name { color: ${fieldData[shownColors[i]]} }`;
    $(`<style>${colors}</style>`).appendTo('head');
});

const testMessage = () => window.dispatchEvent(new CustomEvent('onEventReceived', { detail: { listener: 'message', event: { data: { time: Date.now(), displayName: ['numiieo', 'dunkston', 'randomname', 'TwitchUser12', 'Chatter', 'SUPERLONGUSERNAME'][Math.floor(Math.random() * 6 + 0)], text: 'Ut enim ad minim, sed love eiusmod. Consectetur adipiscing elit! tempor incididunt ut labore et dolore magna aliqua.'.split(' ').slice(0, Math.floor(Math.random() * 18 + 1)).join(' '), msgId: '1', userId: '1', tags: { badges: ''}}}}}));

const addMessage = data => $('body').prepend(`<div class="message-container container" data-msgId="${data.msgId}" data-userId="${data.userId}"><p class="message-name">${data.displayName.toUpperCase()}</p>${fieldData.transparent ? '' : '<div class="bell-container"><svg class="bell" viewBox="0 0 60 66"><path d="M19.998,54.82c-1.771,-0 -3.075,1.858 -2.279,3.441c2.712,5.388 8.779,8.521 14.97,7.263c4.071,-0.825 7.471,-3.429 9.363,-6.975c0.9,-1.684 -0.242,-3.729 -2.15,-3.729l-19.904,-0Z" style="fill-rule:nonzero;"/><path d="M8.221,49.338l42.913,-0.001c4.542,0 8.225,-3.683 8.225,-8.22c-0,-0.955 -0.167,-1.905 -0.492,-2.8l-7.816,-21.613c-2.746,-9.871 -11.734,-16.704 -21.98,-16.704c-10.687,-0 -19.945,7.425 -22.262,17.858l-6.442,20.834c-1.337,4.341 1.092,8.941 5.434,10.279c0.783,0.242 1.6,0.366 2.42,0.366" style="fill-rule:nonzero;"/></svg></div>'}<p class="message-text">${handleEmotes((fieldData.allCaps ? data.text.toUpperCase() : data.text), data.emotes)}</p></div>`);
...
const handleEmotes = (text, emotes) => {
    ...
    for(let emote of emotes) {
        ...
        text = `${text.slice(0, emote.start - offset)}${replacementText}${text.slice(emote.end - offset + 1)}`;
        offset += emote.end + 1 - emote.start  - replacementText.length;
    }
    return text;
}

const clearMessageLog = () => { if(document.body.children.length > 19 + shownColors.length) for(let i = 19 + shownColors.length; i > 19; i--) document.body.children[i].remove() }

const animateContainers = () => {
    let newContainer = $('.container')[0];
    newContainer.style.marginBottom = `-${newContainer.scrollHeight + parseFloat(getComputedStyle(newContainer).marginTop)}px`;
...
const hideMessageOverflow = () => {
    if(fieldData.hideMessageOverflow) {
        ...
        $('.container').each((index, value) => {
            ...
            stackHeight += value.scrollHeight + parseFloat(getComputedStyle(value).marginTop);
            let offset = stackHeight - window.innerHeight - parseFloat(getComputedStyle(value).marginTop);

            if(offset > 2) {
                value.style.marginBottom = `${value.scrollHeight - offset + parseFloat(getComputedStyle(value).marginTop) + 1}px`;
                alreadyPushed = true;
            }
        });
    }
...
const addAlert = (data, event) => {
    ...
    $('body').prepend(`<div class="alert-container container"><div class="icon-container"><svg class="icon" viewBox="0 0 36 36">${icons[event]}</svg></div>${fieldData.transparent ? '' : '<div class="bell-container"><svg class="bell" viewBox="0 0 60 66"><path d="M19.998,54.82c-1.771,-0 -3.075,1.858 -2.279,3.441c2.712,5.388 8.779,8.521 14.97,7.263c4.071,-0.825 7.471,-3.429 9.363,-6.975c0.9,-1.684 -0.242,-3.729 -2.15,-3.729l-19.904,-0Z" style="fill-rule:nonzero;"/><path d="M8.221,49.338l42.913,-0.001c4.542,0 8.225,-3.683 8.225,-8.22c-0,-0.955 -0.167,-1.905 -0.492,-2.8l-7.816,-21.613c-2.746,-9.871 -11.734,-16.704 -21.98,-16.704c-10.687,-0 -19.945,7.425 -22.262,17.858l-6.442,20.834c-1.337,4.341 1.092,8.941 5.434,10.279c0.783,0.242 1.6,0.366 2.42,0.366" style="fill-rule:nonzero;"/></svg></div>'}<p class="alert-name">${name.toUpperCase()}</p><p class="alert-text">${(fieldData.allCaps ? action.toUpperCase() : action)}</p></div>`);
    return true;
}